        ...
        <div v-if="isReady">
          ...
          <template v-if="state == 'SELECTRANGE'">
            ...
            <div class="mt-4">
              <SelectNative
                title="排序方式"
                v-model="weiboSortType"
                :options="OptionsWeiboSortType"
              />
            </div>
            <div class="mt-4">
              <button
          ...
          <template v-if="state == 'SAVING'">
            ...
            <div class="border-t border-gray-200 mt-4 pt-4">
              <div class="flex gap-3">
                ...
                <button
                  type="button"
                  @click="eventSaveMarkdown"
                  :disabled="isExportingMarkdown"
                  class="w-full flex justify-center py-2 px-4 border border-orange-200 rounded-md shadow-sm text-sm font-medium text-orange-700 bg-orange-50 hover:bg-orange-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {{
                    isExportingMarkdown
                      ? "正在导出..."
...
import { exportMarkdownPackage } from "./module/exportMarkdown"

const OptionsWeiboTimeRange = ["全部时间", "指定时间"]
const OptionsWeiboSourceType = ["全部微博", "原创微博"]
const OptionsWeiboSortType = ["时间倒序（新到旧）", "时间正序（旧到新）"]
...
export default {
  ...
  methods: {
    ...
    async eventFetchPosts() {
      ...
      if (this.weiboSortType == 1) {
        this.pendingWording += `，按时间正序`
        documentTitle += `_正序`
      } else {
        this.pendingWording += `，按时间倒序`
        documentTitle += `_倒序`
      }

      document.title = documentTitle
      this.exportFileBase = documentTitle

      let fetchTask = await fetchPost(
        {
          uid: this.uid,
          sourceType: this.weiboSourceType, //
          sortType: this.weiboSortType, //
    ...
    async eventSaveMarkdown() {
      if (this.isExportingMarkdown) {
        return
      }

      this.isExportingMarkdown = true
      try {
        await exportMarkdownPackage({
          posts: this.posts,
          username: this.username,
          fileBase: this.exportFileBase || `@${this.username}`,
        })
        this.state = "DONE"
      } catch (error) {
        console.error(error)
        window.alert("Markdown 导出失败，请稍后重试。")
      } finally {
